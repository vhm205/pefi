...
// Add these options (you might want to move them to a shared constants file)
...
export default function BudgetModal({
  ...
  useEffect(() => {
    if (budget) {
      setFormData(budget);
      setDisplayAmount(formatMoney(budget.amount.toString()));
    } else {
      // Set default fund to "everyday" for new budgets
      setFormData({
  ...
  return (
    <dialog className={`modal ${isOpen ? "modal-open" : ""}`}>
      <div className="modal-box max-w-xl">
        ...
        <h3 className="text-lg font-bold mb-6">Edit Budget</h3>
